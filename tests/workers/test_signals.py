...
def test_pending_signal_includes_importance():
    ...
    assert "importance" in dict_content, "add_pending_signal dict must include 'importance' key"
...
def test_pending_signal_includes_durability():
    ...
    assert match, "Could not find add_pending_signal call"
    dict_content = match.group(1)
    assert "durability" in dict_content, "add_pending_signal dict must include 'durability' key"
