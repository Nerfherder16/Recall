...
def test_eval_signal_classifier_produces_metrics():
    """eval_signal_classifier returns valid metrics dict."""
    with open(DATASETS_DIR / "signal_eval.json") as f:
        data = json.load(f)
...
def test_eval_signal_classifier_f1_above_threshold():
    """Signal baseline should achieve F1 > 0.6."""
    with open(DATASETS_DIR / "signal_eval.json") as f:
        data = json.load(f)
    metrics = eval_signal_classifier(data)
    assert metrics["f1"] > 0.6, f"F1={metrics['f1']} below 0.6 threshold"
