...
class StressSuite(BaseSuite):
    ...
    async def run(self) -> SuiteReport:
        ...
        try:
            ...
            for round_num in range(3):
                ...
                self.observe(f"  Round {round_num}: {successes}/{concurrency} ok, {ops_per_sec:.1f} ops/s")
            ...
            search_queries = [
                "concurrent write throughput", "stress test parallel",
                "unique content verification", "system handles correctly",
                "testing concurrent stores", "random salt value",
                "write throughput test", "parallel store operations",
                "verify system handles", "stress test memory",
            ]
            ...
            search_successes = sum(1 for r in search_results if not isinstance(r, Exception) and r is not None)
            search_ops = search_successes / search_elapsed if search_elapsed > 0 else 0

            self.metric("concurrent_searches", {
            ...
            self.observe(f"Concurrent searches: {search_successes}/{concurrency} ok, {search_ops:.1f} ops/s")
            ...
            for i in range(total_ops):
                if random.random() < 0.7:
                    mixed_tasks.append(self._sem_search(sem, f"stress test memory {i}", read_latencies))
                else:
                    content = f"Mixed workload write {i}: random content {random.randint(10000, 99999)}"
            ...
            self.metric("mixed_workload", {
                "total_ops": total_ops,
                "duration_seconds": round(mixed_elapsed, 2),
                "throughput_ops_per_sec": round(total_ops / mixed_elapsed, 2) if mixed_elapsed > 0 else 0,
                "read_latency_p50_ms": round(_percentile(read_latencies, 0.5) * 1000, 1) if read_latencies else 0,
                "read_latency_p95_ms": round(_percentile(read_latencies, 0.95) * 1000, 1) if read_latencies else 0,
                "write_latency_p50_ms": round(_percentile(write_latencies, 0.5) * 1000, 1) if write_latencies else 0,
                "write_latency_p95_ms": round(_percentile(write_latencies, 0.95) * 1000, 1) if write_latencies else 0,
            })
            ...
            dedup_content = f"Dedup test: this exact content should only be stored once. Run ID: {self.client.run_id}"
            ...
            if created_count > 1:
                self.observe(f"Warning: {created_count} creates (expected 1) — race condition in dedup check")
            ...
            batch_items = [
                {
                    "content": f"Batch-under-load item {i}: testing batch operations while searches run. Salt: {random.randint(10000, 99999)}",
            ...
            search_during_batch = [
                self._timed_search(f"batch operations test {i}")
                for i in range(5)
            ]
            ...
            search_during_results = await asyncio.gather(*search_tasks_during, return_exceptions=True)

            batch_created = 0
            if batch_result and "results" in batch_result:
                batch_created = sum(1 for r in batch_result["results"] if r.get("created"))

            self.metric("batch_under_load", {
                "batch_latency_ms": round(batch_latency * 1000, 1),
                "batch_items_created": batch_created,
                "concurrent_searches_completed": sum(
                    1 for r in search_during_results
                    if not isinstance(r, Exception) and r is not None
                ),
            })
            self.observe(f"Batch: {batch_created} created in {batch_latency*1000:.0f}ms with 5 concurrent searches")
    ...
    async def _timed_search(self, query: str) -> list[dict] | None:
        return await self.client.search_query(query, limit=5, domains=[self.domain])
    ...
    async def _sem_search(self, sem: asyncio.Semaphore, query: str, latencies: list):
        async with sem:
            t = time.monotonic()
            await self.client.search_query(query, limit=5, domains=[self.domain])
